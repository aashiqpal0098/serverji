    ...
    if not story_text or not st.session_state.img_path:
        ...
        with st.spinner("वीडियो बन रहा है... 1-2 मिनट"):
            try:
                ...
                # Get duration
                dur_out = subprocess.run(["ffprobe","-v","error","-show_entries","format=duration","-of","default=noprint_wrappers=1:nokey=1",audio_file], capture_output=True, text=True)
                ...
                st.download_button("⬇️ डाउनलोड", data=video_bytes, file_name="AASHIQ_Video.mp4")
